...
namespace LibVLCSharp
{
    ...
    public class VLCMediaPlayer : VLCVideoProviderBase
    {
        ...
        public static bool IsVLCAvailable { get; private set; } = true;
        ...
        private void Awake()
        {
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogWarning("[VLC] Failed to initialize libvlc, VLC support disabled: " + ex.Message);
                enabled = false;
                return;
            }
        ...
        public void Stop()
        {
            ...
            {
                Debug.LogWarning("[VLC] Error stopping media player: " + ex.Message);
            }
        }
        ...
        void CreateLibVLC()
        {
            ...
            Core.Initialize(); // Use system-installed libvlc on Linux
#else
