...
namespace DtxCS
{
    ...
    public static class DTX
    {
        ...
        public static DataArray FromPlainTextBytes(byte[] dtaBytes)
        {
            ...
            {
                if (d != null && d is DataArray && ((DataArray) d).Array("encoding") != null &&
                    ((DataArray) d).Array("encoding").Children[1].Name == "utf8")
                {
                    utf8 = true;
                    break;
                }
                        ...
                        switch (data[i])
                        {
                            ...
                            case ']':
                                ...
                                switch (tmp_directive)
                                {
                                    ...
                                    default:
                                        Console.WriteLine(
                                            $"Unknown directive {tmp_directive}, constant {tmp_constant}");
        ...
        public static int DtbVersion(System.IO.Stream dtb, ref bool encrypted)
        {
            ...
            if (dtb.ReadUInt8() != 0x01)
            {
                dtb.Position = 0;
                dtb = new CryptStream(dtb);
                if (dtb.ReadUInt8() != 0x01) throw new Exception("DTB contained unrecognized header.");
                encrypted = true;
            }
            ...
            for (int i = 0; i < numChildren; i++)
            {
                ...
                switch (t)
                {
                    ...
                    case DataType.MACRO:
                        ...
                        write_children((DataArray) (dta.Children[i]), dtb, ((DataArray) dta.Children[i]).Count,
                            version);
                        break;
                    ...
                    default:
                        throw new Exception(
                            $"Unhandled DTB DataType {Enum.GetName(typeof(DataType), t)} ({(uint) t}) in write_children");
                }
            }
        ...
        static DataArray parse_children(System.IO.Stream s, uint numChildren, DataType type = DataType.ARRAY,
            int version = 1)
        {
            ...
            while (numChildren-- > 0)
            {
                ...
                switch (t)
                {
                    ...
                    default:
                        throw new Exception(
                            $"Unhandled DTB DataType {Enum.GetName(typeof(DataType), t)} ({(uint) t}) in parse_children");
                }
            }
