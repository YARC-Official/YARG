...
namespace YARG.Serialization {
	public static class XboxRawfileBrowser {
		public static List<XboxSong> BrowseFolder(DirectoryInfo folder) {
			...
			try {
				using var sr = new StreamReader(Path.Combine(folder.FullName, "songs.dta"), Encoding.GetEncoding("iso-8859-1"));
			...
			for (int i = 0; i < dtaTree.Count; i++) {
				try {
					...
					var currentSong = new XboxSong(folder.ToString(), currentArray);
