...
namespace YARG.Serialization {
    public static class XboxPNGParser {
        ...
        private static void DecompressBlockDXT3(uint x, uint y, uint width, byte[] blockStorage, uint[] image){
            byte[] alphaValues = new byte[16];

            for(int i = 0; i < 4; i++){
                ushort alphaData = BitConverter.ToUInt16(blockStorage, 0);
                alphaValues[i*4] = (byte)((alphaData & 0xF) * 17);
                ...
                blockStorage = blockStorage.Skip(2).ToArray();
            }
                ...
                for(int i = 0; i < 4; i++){
                    ...
                    switch(positionCode){
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            if(color0 > color1) finalColor = PackRGBA((byte)((2*r0+r1)/3), (byte)((2*g0+g1)/3), (byte)((2*b0+b1)/3), alpha);
                            else finalColor = PackRGBA((byte)((r0+r1)/2), (byte)((g0+g1)/2), (byte)((b0+b1)/2), alpha);
                            break;
                        case 3:
                            if(color0 > color1) finalColor = PackRGBA((byte)((r0+2*r1)/3), (byte)((g0+2*g1)/3), (byte)((b0+2*b1)/3), alpha);
                            else finalColor = PackRGBA(0, 0, 0, alpha);
                            break;
                    }
        ...
        public static void BlockDecompressImageDXT3(uint width, uint height, byte[] blockStorage, uint[] image){
            uint blockCountX = (width + 3) / 4;
            uint blockCountY = (height + 3) / 4;

            for(uint j = 0; j < blockCountY; j++){
                for(uint i = 0; i < blockCountX; i++){
                    DecompressBlockDXT3(i*4, j*4, width, blockStorage, image);
                    blockStorage = blockStorage.Skip(16).ToArray();
                }
            }
        }
    }
