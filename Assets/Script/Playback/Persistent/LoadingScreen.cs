...
namespace YARG
{
    ...
    public sealed class LoadingContext : IDisposable
    {
        ...
        {
            Debug.Log($"[LoadingContext.Wait] Starting - {_queue.Count} tasks in queue");
            int taskCount = 0;
            while (_queue.TryDequeue(out var node))
            {
                taskCount++;
                Debug.Log($"[LoadingContext.Wait] Processing task {taskCount}: {node.Text}");
                SetLoadingText(node.Text, node.SubText);

                try
                {
                    await node.Task;
                    Debug.Log($"[LoadingContext.Wait] Task {taskCount} completed: {node.Text}");
                }
                catch (Exception ex)
                {
                    Debug.LogError($"[LoadingContext.Wait] Task {taskCount} failed: {node.Text}");
                    YargLogger.LogException(ex);
                }
            }
            Debug.Log("[LoadingContext.Wait] All tasks completed, running GC");
            GC.Collect();
            Debug.Log("[LoadingContext.Wait] Wait completed");
        }

        public async void Dispose()
        {
            Debug.Log("[LoadingContext] Dispose called");
            if (!_disposed)
            {
                Debug.Log("[LoadingContext] Waiting for remaining tasks...");
                await Wait();
                Debug.Log("[LoadingContext] Tasks complete, hiding loading screen");
                LoadingScreen.Instance.gameObject.SetActive(false);
                Navigator.Instance.DisableMenuInputs = false;
                _disposed = true;
                Debug.Log("[LoadingContext] Dispose complete - loading screen hidden");
            }
            else
            {
                Debug.LogWarning("[LoadingContext] Dispose called but already disposed");
            }
