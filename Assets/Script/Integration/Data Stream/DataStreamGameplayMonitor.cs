...
namespace YARG.Integration
{
    public class DataStreamGameplayMonitor : GameplayBehaviour
    {
        ...
        protected override void OnChartLoaded(SongChart chart)
        {
            ...
            DataStreamController.MLCVenue = (DataStreamController.VenueType)Random.Range(1, 2);
        ...
        private void Update()
        {
            ...
            {
                DataStreamController.MLCPaused = DataStreamController.PauseStateType.Paused;
                //if paused force the strobe and fog off, so no one dies.
            ...
            else
            {
                ...
                DataStreamController.MLCStrobeState = _currentStrobeState;
                DataStreamController.MLCFogState = _currentFogState;
            }
            ...
            while (LightingIndex < Venue.Lighting.Count && Venue.Lighting[LightingIndex].Time <= GameManager.SongTime)
            {
                switch (Venue.Lighting[LightingIndex].Type)
                {
                    ...
                    case LightingType.StrobeFast:
                        DataStreamController.MLCStrobeState = LightingType.StrobeFast;
                        break;

                    case LightingType.StrobeMedium:
                        DataStreamController.MLCStrobeState = LightingType.StrobeMedium;
                        break;

                    case LightingType.StrobeSlow:
                        DataStreamController.MLCStrobeState = LightingType.StrobeSlow;
                        break;

                    case LightingType.StrobeFastest:
                        DataStreamController.MLCStrobeState = LightingType.StrobeFastest;
                        break;
            ...
            while (_stageIndex < Venue.Stage.Count && Venue.Stage[_stageIndex].Time <= GameManager.SongTime)
            {
                ...
                {
                    DataStreamController.MLCFogState = true;
                }
                ...
                {
                    DataStreamController.MLCBonusFX = true;
                }
