...
namespace YARG.UI.MusicLibrary {
	public class SelectedSongView : MonoBehaviour {
		...
		private void LoadAlbumCover() {
			...
			else if(_songEntry.SongType == SongType.RbCon){
				var c = (ConSongEntry)_songEntry;
				if(c.ImagePath != string.Empty){
					albumCover.texture = XboxImageTextureGenerator.GetTexture(
			...
			else if(_songEntry.SongType == SongType.ExtractedRbCon){
				var x = (ExtractedConSongEntry)_songEntry;
				if(x.ImagePath != string.Empty){
					albumCover.texture = XboxImageTextureGenerator.GetTexture(File.ReadAllBytes(x.ImagePath));
					albumCover.color = Color.white;
					albumCover.uvRect = new Rect(0f, 0f, 1f, -1f);
				}
			}
