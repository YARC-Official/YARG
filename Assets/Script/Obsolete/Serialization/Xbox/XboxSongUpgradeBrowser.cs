...
namespace YARG.Serialization
{
    ...
    {
        public static List<(SongProUpgrade, DataArray)> FetchSongUpgrades(string upgrade_folder,
            ref List<XboxSTFSFile> conFiles)
        {
            DataArray dtaTree;
            ...
            if (File.Exists(Path.Combine(upgrade_folder, "upgrades.dta")))
            {
                ...
                for (int i = 0; i < dtaTree.Count; i++)
                {
                    try
                    {
                        songUpgrades.Add(new(new SongProUpgrade(upgrade_folder, dtaTree[i].Name),
                            (DataArray) dtaTree[i]));
                    }
                    catch (Exception e)
                    {
