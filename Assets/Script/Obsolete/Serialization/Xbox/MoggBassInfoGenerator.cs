        ...
        public static void Generate(ExtractedConSongEntry song, DataArray dta, List<DataArray> dta_update_roots = null)
        {
                        ...
                        case "tracks":
                            ...
                            for (int x = 0; x < trackArray.Count; x++)
                            {
                                ...
                                if (instrArray[1] is DataArray trackNums)
                                {
                                    ...
                                    for (int y = 0; y < trackNums.Count; y++) val[y] = ((DataAtom) trackNums[y]).Int;
                                    Tracks.Add(key, val);
                                }
                        ...
                        case "vols":
                            ...
                            for (int v = 0; v < volArray.Count; v++)
                            {
                                ...
                                if (volAtom.Type == DataType.FLOAT)
                                    VolumeData[v] = ((DataAtom) volArray[v]).Float;
                                else
                                    VolumeData[v] = ((DataAtom) volArray[v]).Int;
                            }
            ...
            if (Tracks.TryGetValue("drum", out var drumArray))
            {
                switch (drumArray.Length)
                {
                    ...
                    case 2:
                        song.StemMaps[SongStem.Drums] = new[]
                        {
                            drumArray[0], drumArray[1]
                        };
                        break;
                    //drum (0 1 2): mono kick, stereo snare/kit --> (0) (1 2)
                    case 3:
                        song.StemMaps[SongStem.Drums1] = new[]
                        {
                            drumArray[0]
                        };
                        song.StemMaps[SongStem.Drums2] = new[]
                        {
                            drumArray[1], drumArray[2]
                        };
                        break;
                    ...
                    case 4:
                        ...
                        song.StemMaps[SongStem.Drums3] = new[]
                        {
                            drumArray[2], drumArray[3]
                        };
                        break;
