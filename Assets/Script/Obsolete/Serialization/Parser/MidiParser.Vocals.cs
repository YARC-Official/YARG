...
namespace YARG.Serialization.Parser
{
    public partial class MidiParser : AbstractParser
    {
        ...
        private List<LyricInfo> ParseRealLyrics(List<EventIR> eventIR, TrackChunk trackChunk,
            TrackChunk phraseTimingTrack, TempoMap tempo, int harmonyIndex)
        {
            ...
            foreach (var note in phraseTimingTrack.GetNotes())
            {
                ...
                if (harmonyIndex == 0)
                {
                    ...
                    {
                        startTick = note.EndTime, name = "harmVocal_endPhrase"
                    });
                }
                else if (harmonyIndex == -1)
                {
                    eventIR.Add(new EventIR
                    {
                        startTick = note.EndTime, name = "vocal_endPhrase"
                    });
                }
            }
