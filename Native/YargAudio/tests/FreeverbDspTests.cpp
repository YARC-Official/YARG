...
yarg_freeverb_dsp* attach(BassCoreBindings& bass, MockBass& state,
    ...
    mock = &state;
    yarg_freeverb_dsp* dsp = nullptr;
    int bassError = -1;
    REQUIRE(freeverbDspAttach(bass, 11, dryMix, wetMix, roomSize, damp, width,
        3, &dsp, &bassError) == YARG_AUDIO_OK);
    REQUIRE(dsp != nullptr);
    REQUIRE(bassError == 0);
...
void testDryPathAndMalformedBuffers() {
    ...
    REQUIRE(freeverbDspDestroy(dsp));
}
...
void testWetPathSilenceAndReset() {
    ...
    std::vector<float> impulse(2000 * state.channels, 0.0f);
    impulse[0] = 1.0f;
    state.callback(17, 11, impulse.data(),
        static_cast<std::uint32_t>(impulse.size() * sizeof(float)), state.callbackUser);
    ...
    REQUIRE(freeverbDspRequestReset(dsp) == YARG_AUDIO_OK);
    ...
    REQUIRE(freeverbDspRequestReset(nullptr) == YARG_AUDIO_ERROR_INVALID_ARGUMENT);
    REQUIRE(freeverbDspDestroy(dsp));
}
...
void testChannelShapes() {
    for (std::uint32_t channels : {1u, 2u, 3u, 6u}) {
        ...
        for (std::size_t i = 0; i < samples.size(); ++i) {
            samples[i] = static_cast<float>(i) * 0.25f - 1.0f;
        }
        const auto expected = samples;
        state.callback(17, 11, samples.data(),
            static_cast<std::uint32_t>(samples.size() * sizeof(float)), state.callbackUser);
...
void testAttachValidation() {
    ...
    mock = &state;
    BassCoreBindings bass(completeFunctions());
    yarg_freeverb_dsp* dsp = reinterpret_cast<yarg_freeverb_dsp*>(1);
    int bassError = -1;

    REQUIRE(freeverbDspAttach(bass, 0, 0, 1, 0, 0, 1, 0, &dsp, &bassError) ==
        YARG_AUDIO_ERROR_INVALID_ARGUMENT);
    REQUIRE(dsp == nullptr && bassError == 0);
    REQUIRE(freeverbDspAttach(bass, 1, std::numeric_limits<float>::quiet_NaN(),
        1, 0, 0, 1, 0, &dsp, &bassError) == YARG_AUDIO_ERROR_INVALID_ARGUMENT);
    REQUIRE(freeverbDspAttach(bass, 1, 0, std::numeric_limits<float>::infinity(),
        0, 0, 1, 0, &dsp, &bassError) == YARG_AUDIO_ERROR_INVALID_ARGUMENT);

    auto incomplete = completeFunctions();
    incomplete.channelSetDsp = nullptr;
    BassCoreBindings missing(incomplete);
    REQUIRE(freeverbDspAttach(missing, 1, 0, 1, 0, 0, 1, 0, &dsp, &bassError) ==
        YARG_AUDIO_ERROR_DEPENDENCY);

    state.infoSucceeds = false;
    REQUIRE(freeverbDspAttach(bass, 1, 0, 1, 0, 0, 1, 0, &dsp, &bassError) ==
    ...
    REQUIRE(freeverbDspAttach(bass, 1, 0, 1, 0, 0, 1, 0, &dsp, &bassError) == YARG_AUDIO_OK);
    REQUIRE(freeverbDspDestroy(dsp));
